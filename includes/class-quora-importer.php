...
class Quora_Importer {
                ...
                <div class="quora-form-section">
                    ...
                    <div class="quora-form-row">
                        ...
                        <div>
                            ...
                            <span class="help-desc" style="display: block; margin-top: 4px;"><?php esc_html_e( "must match the name on Quora if the 'Link to Quora' option below is enabled", 'quora-importer' ); ?></span>
                        </div>
                    </div>
                    ...
                    <div class="quora-form-row">
                        ...
                        <select name="link_position" id="quora-link-position">
                            <option value="none" <?php if ( ! class_exists( 'Quora_Importer_Extended' ) ) echo 'selected="selected"'; ?>><?php esc_html_e( 'No', 'quora-importer' ); ?></option>
                            <option value="top" <?php if ( class_exists( 'Quora_Importer_Extended' ) ) echo 'selected="selected"'; ?>><?php esc_html_e( 'At the top of the post', 'quora-importer' ); ?></option>
                            <option value="bottom"><?php esc_html_e( 'At the bottom of the post', 'quora-importer' ); ?></option>
                        </select>
                    </div>
                    
                    <div class="quora-form-row" id="quora-link-template-row" <?php if ( ! class_exists( 'Quora_Importer_Extended' ) ) echo 'style="display: none;"'; ?>>
    ...
    public function ajax_import_item() {
        ...
        $extract_topics = ! empty( $_POST['extract_topics'] ) && class_exists( 'Quora_Importer_Extended' );
        $r2w_support = ! empty( $_POST['r2w_support'] );
        update_option( 'quora_importer_r2w_support', $r2w_support ? '1' : '0' );
        $import_comments = class_exists( 'Quora_Importer_Extended' ) ? ( isset( $_POST['import_comments'] ) ? sanitize_text_field( wp_unslash( $_POST['import_comments'] ) ) : 'none' ) : 'none';
        ...
        if ( $existing ) {
            ...
            if ( $extract_topics ) {
                if ( ! empty( $extracted_topics ) || ( ! empty( $quora_url ) && Quora_Importer_Extended::test_url_status( $quora_url ) === 200 ) ) {
                    ...
                    if ( ! empty( $quora_url ) ) {
                        update_post_meta( $existing->ID, '_quora_url', $quora_url );
                        $status_code = Quora_Importer_Extended::test_url_status( $quora_url );
                        if ( $status_code === 404 ) {
            ...
            if ( $extract_topics ) {
                if ( ! empty( $extracted_topics ) ) {
                    ...
                    if ( ! empty( $initial_url ) ) {
                        $status_code = Quora_Importer_Extended::test_url_status( $initial_url );
                        if ( $status_code === 404 ) {
        ...
        if ( $is_published && 'none' !== $import_comments ) {
            if ( 'direct' === $import_comments ) {
                if ( ! empty( $quora_urls ) ) {
                    $comments_res = Quora_Importer_Extended::import_comments( $post_id, $quora_urls, $post_date );
    ...
    /**
     * Locate the python executable on the host system.
     */
    ...
    private function append_suffix_to_url( $url, $suffix ) {
        if ( substr( $url, -1 ) === '/' ) {
            return $url . trim( $suffix, '/' );
        } else {
     ...
     */
    public function log_404( $title, $url ) {
        $upload_dir = wp_upload_dir();
    ...
    private function find_best_quora_url( $post, $extracted_dir, $author_id, $is_published, $extract_topics, &$extracted_topics, &$candidate_urls ) {
        ...
        if ( ! class_exists( 'Quora_Importer_Extended' ) ) {
        ...
        if ( $extract_topics ) {
            ...
            foreach ( $candidate_urls as $candidate ) {
                ...
                if ( $status_code === 200 || ( $status_code > 0 && $status_code !== 404 ) ) {
                    ...
                    $this->log_404( $title_for_log, $candidate );
                }
            }
        }
        ...
        // Detect and save successful method
        if ( ! empty( $quora_url ) ) {
            $title = ! empty( $post['Question'] ) ? $post['Question'] : ( ! empty( $post['Title'] ) ? $post['Title'] : '' );
            $direct_url = $this->generate_quora_url( $post, $extracted_dir, $author_id, false, false );
            $url_a = ! empty( $title ) ? $this->generate_quora_url( $post, $extracted_dir, $author_id, true, true ) : '';
            $url_b = ! empty( $title ) ? $this->generate_quora_url( $post, $extracted_dir, $author_id, false, true ) : '';
            
            $success_method = 'direct';
            if ( ! empty( $url_b ) && ( $quora_url === $url_b || strpos( $quora_url, $url_b . '-' ) === 0 || strpos( $quora_url, rtrim( $url_b, '/' ) . '-' ) === 0 ) ) {
                $success_method = 'slug_b';
            } elseif ( ! empty( $url_a ) && ( $quora_url === $url_a || strpos( $quora_url, $url_a . '-' ) === 0 || strpos( $quora_url, rtrim( $url_a, '/' ) . '-' ) === 0 ) ) {
                $success_method = 'slug_a';
            }
            
            update_option( 'quora_importer_last_success_method', $success_method );
        }
    ...
    private function get_candidate_urls( $post, $extracted_dir, $author_id = 0 ) {
        ...
        if ( ! empty( $direct_url ) && $this->validate_quora_url( $direct_url ) ) {
            $groups['direct'][] = $direct_url;
            $groups['direct'][] = $this->append_suffix_to_url( $direct_url, '-1' );
            $groups['direct'][] = $this->append_suffix_to_url( $direct_url, '-2' );
        }
        
        if ( ! empty( $url_a ) && $this->validate_quora_url( $url_a ) ) {
            $groups['slug_a'][] = $url_a;
            $groups['slug_a'][] = $this->append_suffix_to_url( $url_a, '-1' );
            $groups['slug_a'][] = $this->append_suffix_to_url( $url_a, '-2' );
        }
        
        if ( ! empty( $url_b ) && $this->validate_quora_url( $url_b ) ) {
            $groups['slug_b'][] = $url_b;
            $groups['slug_b'][] = $this->append_suffix_to_url( $url_b, '-1' );
            $groups['slug_b'][] = $this->append_suffix_to_url( $url_b, '-2' );
        }
        
        // Order groups adaptively based on the last successful method
        $last_success_method = get_option( 'quora_importer_last_success_method', 'direct' );
        $method_order = array( 'direct', 'slug_a', 'slug_b' );
        if ( in_array( $last_success_method, $method_order ) ) {
            $method_order = array_diff( $method_order, array( $last_success_method ) );
            array_unshift( $method_order, $last_success_method );
        }
        
        $urls = array();
        foreach ( $method_order as $method ) {
            foreach ( $groups[$method] as $url ) {
     ...
     */
    public function decode_shell_json( $output_array ) {
     ...
     */
    public function import_quora_comments_to_post( $post_id, $comments, $post_date ) {
        $id_map = array();
    ...
    public function run_database_updates() {
        ...
        $test_url_status = function( $url ) {
            if ( class_exists( 'Quora_Importer_Extended' ) ) {
                return Quora_Importer_Extended::test_url_status( $url );
            }
            return 0;
        };
            ...
            foreach ( $parsed_posts as $post ) {
                ...
                if ( $matched_post ) {
                    ...
                    if ( ! empty( $candidate_urls ) ) {
                        ...
                        if ( $is_valid_dict ) {
                        ...
                        } elseif ( class_exists( 'Quora_Importer_Extended' ) ) {
                            ...
                            if ( $status === 'valid' ) {
                                $slug_a = $this->quora_slugify( $clean_title, true );
                                $slug_b = $this->quora_slugify( $clean_title, false );
                                $success_method = 'direct';
                                if ( ! empty( $slug_b ) && strpos( $url_to_test, $slug_b ) !== false ) {
                                    $success_method = 'slug_b';
                                } elseif ( ! empty( $slug_a ) && strpos( $url_to_test, $slug_a ) !== false ) {
                                    $success_method = 'slug_a';
                                }
                                update_option( 'quora_importer_last_success_method', $success_method );
                            }
        ...
        foreach ( $invalid_posts as $db_post ) {
            ...
            $add_slug_candidates = function( $t ) use ( &$candidates, $domain, $profile_slugs ) {
                $last_success_method = get_option( 'quora_importer_last_success_method', 'direct' );
                $loop_order = array( true, false );
                if ( $last_success_method === 'slug_b' ) {
                    $loop_order = array( false, true );
                }
                foreach ( $loop_order as $replace_apostrophes ) {
            ...
            if ( ! $valid_url && class_exists( 'Quora_Importer_Extended' ) ) {
                foreach ( $candidates as $candidate_url ) {
                    ...
                    if ( $status_code === 200 || ( $status_code > 0 && $status_code !== 404 ) ) {
                        ...
                        // Update last success method option
                        $slug_a = $this->quora_slugify( $db_post->post_title, true );
                        $slug_b = $this->quora_slugify( $db_post->post_title, false );
                        $success_method = 'direct';
                        if ( ! empty( $slug_b ) && strpos( $candidate_url, $slug_b ) !== false ) {
                            $success_method = 'slug_b';
                        } elseif ( ! empty( $slug_a ) && strpos( $candidate_url, $slug_a ) !== false ) {
                            $success_method = 'slug_a';
                        }
                        update_option( 'quora_importer_last_success_method', $success_method );
